return {
  ...
  config = function()
    ...
    for i = 1, 9 do
      map("n", "<leader>b" .. i, "<Cmd>BufferGoto " .. i .. "<CR>", vim.tbl_extend("force", opts, {
        desc = "Go to buffer " .. i
      }))
    end

    -- Leader + 0: Go to the last buffer
    map("n", "<leader>b0", "<Cmd>BufferLast<CR>", vim.tbl_extend("force", opts, {
      desc = "Go to last buffer"
    }))

    -- Leader + n: Go to the next buffer
    map("n", "<leader>bn", "<Cmd>BufferNext<CR>", vim.tbl_extend("force", opts, {
      desc = "Next buffer"
    }))

    -- Leader + N: Go to the previous buffer
    map("n", "<leader>bN", "<Cmd>BufferPrevious<CR>", vim.tbl_extend("force", opts, {
      desc = "Previous buffer"
    }))

    -- Leader + c: Close current buffer
    map("n", "<leader>bc", "<Cmd>BufferClose<CR>", vim.tbl_extend("force", opts, {
      desc = "Close buffer"
    }))
  end
}
